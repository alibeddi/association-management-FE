        ...
        <RHFAsyncSelect
          ...
          fetchData={async (params) => {
            const response = await axios.get(
              `/users?page=${params.page}&limit=${params.limit}&filterName=${params.name}`
            );
          ...
          onChange={(users: User[]) => {
            ...
            if (userId) {
              dispatch(handleChangefilter({ id: filter.id, value: userId }));
            }
          }}
          sx={{
            padding: '.5rem 1rem',
    ...
    case MENU_ITEM_VALUE.kpis:
      return (
        <RHFAsyncSelect
          ...
          required
          isSearchable
          getOptionLabel={(option: IKpi) =>
            option && typeof option !== 'string' ? option?.label : option
          }
          getOptionValue={(option) => option}
          fetchData={async (params) => {
            const response = await axios.get(
              `/kpis?page=${params.page}&limit=${params.limit}&filterName=${params.name}`
            );
            const data = await response.data;
            return data;
          }}
          onChange={(e: IKpi) => {
            if (e) dispatch(handleChangefilter({ id: filter.id, value: e }));
          }}
          sx={{
            padding: '.5rem 1rem',
          }}
    ...
    case MENU_ITEM_VALUE.statClient:
      return (
        <RHFAsyncSelect
          ...
          fetchData={async (params) => {
            const response = await axios.get(
              `/stat-clients?page=${params.page}&limit=${params.limit}&filterName=${params.name}`
            );
            const data = await response.data;
            return data;
          }}
